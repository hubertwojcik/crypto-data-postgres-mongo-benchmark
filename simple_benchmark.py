#!/usr/bin/env python3
"""
Prosty skrypt do testów wydajności - krok po kroku z monitoringiem.
"""
...
def safe_test(test_name, test_func, timeout=30):
    """Bezpieczne wykonanie testu z timeout."""
    ...
    try:
        ...
        if elapsed > timeout:
            ...
            return {"time": 0.0, "status": "timeout"}
        
        test_time = result.get("time", elapsed) if isinstance(result, dict) else elapsed
        print(f"✅ {test_time:.6f}s")
        return {"time": test_time, "status": "success"}
    except Exception as e:
        print(f"❌ ERROR: {str(e)[:50]}...")
        return {"time": 0.0, "status": "error", "error": str(e)}

def run_simple_benchmark():
    """Uruchom prosty benchmark krok po kroku."""
    print("🚀 Prosty benchmark PostgreSQL vs MongoDB")
    print("="*60)
    ...
    for dataset_name, csv_path, expected_size in datasets:
        ...
        try:
            ...
            # 5. Testy CRUD - tylko podstawowe
            print(f"\n🧪 Testy CRUD (podstawowe)")
    ...
    for dataset_name, data in results.items():
        ...
        pg_times = [r["postgresql"] for r in crud_results.values() if r["postgresql"] > 0]
        mongo_times = [r["mongodb"] for r in crud_results.values() if r["mongodb"] > 0]
